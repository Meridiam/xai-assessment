...
from tau2.domains.xai_domain.data_model import LineStatus, XaiDomainDB
from tau2.domains.xai_domain.tools import XaiDomainTools
from tau2.domains.xai_domain.user_data_model import PaymentRequest, XaiDomainUserDB
from tau2.domains.xai_domain.user_tools import XaiDomainUserTools
from tau2.domains.xai_domain.utils import (
    XAI_DOMAIN_DB_PATH,
    XAI_DOMAIN_MAIN_POLICY_PATH,
    XAI_DOMAIN_MAIN_POLICY_SOLO_PATH,
    XAI_DOMAIN_TASK_SET_PATH,
    XAI_DOMAIN_TASK_SET_PATH_FULL,
    XAI_DOMAIN_TASK_SET_PATH_SMALL,
    XAI_DOMAIN_TECH_SUPPORT_POLICY_MANUAL_PATH,
    ...
    XAI_DOMAIN_USER_DB_PATH,
)
from tau2.environment.environment import Environment
from tau2.utils import load_file


class XaiDomainEnvironment(Environment):
    tools: XaiDomainTools
    user_tools: XaiDomainUserTools
...
def get_environment(
    db: Optional[XaiDomainDB] = None,
    user_db: Optional[XaiDomainUserDB] = None,
    solo_mode: bool = False,
    policy_type: str = "manual",  # "manual" or "workflow"
) -> XaiDomainEnvironment:
    if db is None:
        db = XaiDomainDB.load(XAI_DOMAIN_DB_PATH)
    tools = XaiDomainTools(db)
    if user_db is None:
        user_db = XaiDomainUserDB.load(XAI_DOMAIN_USER_DB_PATH)
    user_tools = XaiDomainUserTools(user_db)
    if not solo_mode:
        policy_path = XAI_DOMAIN_MAIN_POLICY_PATH
        if policy_type == "manual":
            tech_support_policy_path = XAI_DOMAIN_TECH_SUPPORT_POLICY_MANUAL_PATH
        elif policy_type == "workflow":
            tech_support_policy_path = XAI_DOMAIN_TECH_SUPPORT_POLICY_WORKFLOW_PATH
        else:
            raise ValueError(f"Invalid policy type: {policy_type}")
    else:
        policy_path = XAI_DOMAIN_MAIN_POLICY_SOLO_PATH
        if policy_type == "manual":
            tech_support_policy_path = XAI_DOMAIN_TECH_SUPPORT_POLICY_MANUAL_SOLO_PATH
        elif policy_type == "workflow":
            tech_support_policy_path = XAI_DOMAIN_TECH_SUPPORT_POLICY_WORKFLOW_SOLO_PATH
        else:
            raise ValueError(f"Invalid policy type: {policy_type}")
    ...
    if policy_type == "manual":
        domain_name = "xai_domain"
    else:
        domain_name = "xai_domain-workflow"
    env = XaiDomainEnvironment(
...
def get_tasks_full() -> list[Task]:
    return load_tasks(XAI_DOMAIN_TASK_SET_PATH_FULL)


def get_tasks_small() -> list[Task]:
    return load_tasks(XAI_DOMAIN_TASK_SET_PATH_SMALL)


def get_tasks() -> list[Task]:
    return load_tasks(XAI_DOMAIN_TASK_SET_PATH)
